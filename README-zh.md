...
- 波形和频谱为互斥视图，共用更大、可调高度的打轴显示区。频谱使用真实的 24 kHz PCM，不会把缩略波形峰值当作音频采样。频率数据由浏览器 Worker 计算一次，滚动、缩放、高度和亮度调整均复用结果；单首 PCM 缓存上限为 64 MiB，频谱缓存上限为 16 MiB，换音频时清除其它音频的分析缓存。显示高度、波形振幅和频谱亮度分别保存在本地。
- 节拍线和节拍吸附均默认关闭。可设置 BPM、首拍偏移、点按测速、选择细分拍，或把节拍网格对齐到选中的起点／播放位置；只有附近的点击才吸附，按住 Alt/Option 可临时绕过。这是手动校准的固定速度网格，不是自动识别音乐节拍。同一份已载入音频切换工作区时会保留网格设置，不会把它套用到其它音频。点击仍遵循原有的播放和自动换行设置。
...
本轮 YouTube 维护参照 [yt-dlp 2026.08.19](https://github.com/yt-dlp/yt-dlp/releases/tag/2026.08.19) 的客户端更新，但不把 yt-dlp 或 Python 加为运行依赖。解析器现可读取 gzip 压缩的播放器请求，每次解析都从浏览器刷新嵌入客户端版本，并把播放凭证限定在本次视频；失效的 Android VR／TV-embedded 回退已移除。下载途中刷新地址时还会校验格式一致，避免拼接不同音轨。没有每日监测，也不会执行远程代码：配置在使用时自动刷新，算法变化仍需发布新扩展构建。即使项目版本仍为 v0.4.6，也需要替换已解压扩展、重新加载并刷新页面；包内 `YOUTUBE-RESOLVER.txt` 可用于识别这次更新。
...
Huhu 结果会保留每个非空歌词行及其原始时间。如果生成的分段空行与下一句发生时间冲突，会按设置的时间精度将空行时间限制在相邻歌词之间的可用间隔内，不删除空行，也不移动实际歌词时间。无法保持唯一时间轴或歌词内容不一致时，编辑器保持原样，并明确提示结果校验问题，不再误报为远端对轴失败。已完成的结果仍可在 Huhu 中查看，无需重新创建对轴任务。

发送给两种对轴服务的音频沿用本地文件原名；无损音频转换后只将扩展名改为 `.m4a`。YouTube 链接根据公开的标题／作者信息命名为“歌名 - 歌手.m4a”（或音频实际格式的扩展名），省略频道名末尾的 `- Topic`。名称查询完全在浏览器中完成；查询失败时使用带视频 ID 的文件名，不影响音频载入，也不需要更新扩展。音频直链会保留可识别的原文件名，不把链接中的查询令牌写入文件名。

Huhu 语种默认选择“自动”，也可沿用已保存的手动选择。检测在本地对待提交歌词进行，向 Huhu 发送实际支持的语种，不会发送 `auto`。包含假名时判为日语；整行英文或行内含英文单词的行数达到 3 行时判为日英混合。不含假名时使用相同的 3 行门槛区分中文和中英混合；包含英文且完全没有 CJK 字符时，无论行数多少都判为英语。每行最多计数一次，标点、数字、emoji 和不可识别符号不参与判断，中日文行内的单字母不算英文单词。
