...
- 打轴侧栏可同时清楚显示选中、播放和时间异常状态，窄屏也不会挤掉标签。各工作区恢复自己的页面位置，不再继承其他页面的滚动位置。
- 波形与频谱打轴增加随缩放变化的固定时间标尺；矮屏将次要参数收进“显示设置”。切换视图和调整参数时保留播放状态与打轴进度。
- 逐行编辑器在原文字旁同步标识重复、倒退及无效时间戳，支持上一个／下一个问题导航；保留原生文本选择、输入法输入与撤销。
- 设置和歌词导入支持键盘操作、可见焦点及手机自定义颜色；主题前景色按实际普通与选中表面检查对比度。通知合并重复消息、在同一卡片更新媒体进度，并支持主动关闭。
