...
- Switch line timing between Standard timing and the dedicated Waveform timing workspace. Its timeline stays fixed while audio plays, with a luminous playback cursor, hover-time guide, click marker, and exact result feedback. Normal scrolling moves the timeline; `Shift` + scroll or the zoom slider changes time scale without rebuilding the player. Clicking updates only the current line start so it can be corrected repeatedly; `S` starts continuous playback from the current line, `F`/`Shift+F` pages the timeline, and `G` confirms and selects the next line. Optional click-to-auto-advance and play-from-new-start settings are independent and disabled by default. With play-from-new-start disabled, clicking preserves the previous playing or paused state; enabling it also starts playback when previously paused. Clicking an existing timestamp explicitly plays from that line.
