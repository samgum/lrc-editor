...
- Waveform and spectrogram timing views are mutually exclusive and share a larger, adjustable display. The spectrogram uses real 24 kHz PCM, not decimated waveform peaks. A browser Worker calculates frequency data once; scrolling, zooming, height, and brightness changes reuse it. The single-track PCM cache is capped at 64 MiB and the frequency cache at 16 MiB; changing media clears the other track's cached analysis. Display height and separate waveform-amplitude/spectrum-brightness preferences are saved locally.
- Optional beat lines and magnetic snapping start disabled. Set BPM and the first-beat offset, tap the tempo, choose beat subdivisions, or align the grid with the selected start or playhead. Only nearby clicks snap; Alt/Option temporarily bypasses snapping. This is a manually calibrated fixed-tempo grid, not automatic beat detection. Grid settings survive workspace navigation for the same loaded audio without being applied to another track. Clicking still follows the existing playback and auto-advance settings.
