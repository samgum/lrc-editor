...
The web application is static and keeps lyric text, preferences, and local media processing in the browser. YouTube and Bilibili links are handled by an optional Manifest V3 companion extension, so the project does not require a media-resolution backend or Python runtime.
